...
- **Phase 2 — split into 2a then 2b (brainstormed 2026-06-29):**
  - **▶ Phase 2a — DO THIS NEXT. Spec written, NOT yet planned:** [docs/superpowers/specs/2026-06-29-phase2a-multi-provider-engine-design.md](docs/superpowers/specs/2026-06-29-phase2a-multi-provider-engine-design.md). Multi-provider **engine** (backend): `LLMClient` **Protocol** + `AnthropicClient` (rename of today's class, keeps native server-side web search) + `OpenAICompatClient` (official `openai` SDK, configurable `base_url`); normalized `CompletionResult` return so agents drop the Anthropic `Message` type; **client-side Tavily search loop** in the researcher for non-Anthropic (dispatch on `llm.supports_server_side_search`, no isinstance); per-request provider config (config in body, secret key in header, extends `X-Anthropic-Key`); extend `pricing.py` (cost shown when model known, else "n/a"); minimal run-form provider fields (per-request, not stored). NO LiteLLM/LangChain. **RESUME: review the 2a spec, then `superpowers:writing-plans` → `subagent-driven-development`.**
  - **Phase 2b — later (not yet specced):** encrypted per-user key storage in Supabase + settings UI (save/replace/clear provider+key), wire saved creds into runs, and the global monthly **Tavily quota cap** accounting (deferred from 2a). Reuses 2a's per-request request shape.
  - **2a brainstorm decisions:** Protocol + 2 native clients; server-provided Tavily for non-Anthropic search with a UI hint that an **Anthropic key → built-in web search (recommended, better results)**; cost = tokens always + $ when model is priced; one model drives all 3 roles for non-Anthropic BYOK.
- **Phase 3 (not yet planned):** Deploy live (frontend → Vercel; backend host TBD) + portfolio post linking GitHub + live URL.

Phase 2a needs `writing-plans` before execution; Phase 2b and Phase 3 each need brainstorming → writing-plans. Execute with **superpowers:subagent-driven-development** (or executing-plans).
