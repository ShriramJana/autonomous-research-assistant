        ...
        {isPending ? (
          ...
          <article className="prose prose-sm dark:prose-invert max-w-none">
            <ReactMarkdown
              ...
              components={{
                // Replace `[N]` citation markers in text nodes with clickable sups.
                p: ({ children }) => (
...
// Walks react children (string or array) and replaces `[N]` occurrences in
// string nodes with <Citation /> components resolved against the master list.
function renderWithCitations(children: React.ReactNode, citations: Source[]): React.ReactNode {
  if (children == null) return children;
  if (Array.isArray(children)) {
    return children.map((c, i) => <span key={i}>{renderWithCitations(c, citations)}</span>);
  }
  ...
  while ((match = re.exec(children)) !== null) {
    ...
    if (match.index > lastIdx) {
      out.push(children.slice(lastIdx, match.index));
    }
    out.push(
      <Citation
        key={`${match.index}-${n}`}
